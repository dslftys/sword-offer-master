...
public class constructTree {
    TreeNode contruct(int []pre,int []in) throws Exception {
        if (pre == null || in == null || pre.length <= 0 || in.length <= 0) {
            return null;
        }
        if (pre.length != in.length) {
            throw new Exception("the length are not equal");
        }
        boolean flag=false;
        TreeNode root=new TreeNode(pre[0]);
        for(int i=0;i<in.length;i++){
            if(pre[0] == in[i]){//找到了中间点
                flag=true;
                root.val=pre[0];
                root.left=contruct(Arrays.copyOfRange(pre,1,i+1), Arrays.copyOfRange(in,0,i));
                root.right=contruct(Arrays.copyOfRange(pre,i+2,pre.length),Arrays.copyOfRange(in,i+1,in.length));
            }
        }
        if(!flag){
